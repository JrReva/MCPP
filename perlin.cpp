...
void Perlin::init(void)
{
    ...
    {
        p[i] = i;
        g1[i] = (float)((m_random.GetNext()  % (B + B)) - B) / B;
        for (j = 0 ; j < 2 ; j++)
            g2[i][j] = (float)((m_random.GetNext() % (B + B)) - B) / B;
        normalize2(g2[i]);
        for (j = 0 ; j < 3 ; j++)
            g3[i][j] = (float)((m_random.GetNext() % (B + B)) - B) / B;
        normalize3(g3[i]);
    }
