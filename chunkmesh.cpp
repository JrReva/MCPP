...
ChunkMesh::VertexData* ChunkMesh::ParseVerts(const float* verts, const int& nbData, const int& nbVertex, float& width, float& height, Vector3f& offset, const float& scale)
{
	VertexData* vd = new VertexData[nbVertex];
	
	float minHeight = INT_MAX;
	float maxHeight = INT_MIN;
	float minWidth = INT_MAX;
	float maxWidth = INT_MIN;
	float minPersp = INT_MAX;
	float maxPersp = INT_MIN;

	for(int i = 0; i < nbData; i+=8)
	{
		VertexData v(verts[i] * scale, verts[i+1] *scale, verts[i+2] * scale, verts[i+3], verts[i+4], verts[i+5], verts[i+6], verts[i+7]);
		vd[i/8] = v;

		//On vérifie pour les valeurs extrêmes.
		if(v.x < minWidth)
			minWidth = v.x;
		if(v.x > maxWidth)
			maxWidth = v.x;
